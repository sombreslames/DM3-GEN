function InitPopulation(Pb::Problem,N::Int32,Grasp::Grasp)
   ...
   Population = Array{Individual}(N)
   for i = 1:1:N
      IndexAlpha,Alpha  = ReactiveGrasp(Grasp)
      Population[i]     = Individual(Pb.Bonus,zeros(Int64,Pb.NBvariables),zeros(Int64,0),zeros(Int64,Pb.NBconstraints),zeros(Int64,Pb.NBvariables))
      Population[i]     = GraspConstruction(Pb,Population[i],Alpha)
...
function Mutation(Pb::Problem,Indi::Individual)
...
function RouletteSelection(Population::Vector{Individual},N::Int32,SumObj::Int32)
   Value::Float64 =  0
...
function ParentSelection(Population::Vector{Individual},N::Int32)
...
function BinaryTourmanent(Population::Vector{Individual},N::Int32,Mode::Bool,SumObj::Int32)
...
function CrossoverMethod1(Pb::Problem,Parent1::Individual,Parent2::Individual)
   ...
   for i in 1:1:Pb.NBvariables
      if Mask[i] == 1 && Parent1.Solution[i] == 1
         ChildTemp   = deepcopy(Child)
         Ok,Child    = SetToOne(Pb,Child,i)
         if Ok==false
            Child    = deepcopy(ChildTemp)
         end
